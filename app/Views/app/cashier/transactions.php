        ...
        <div class="flex flex-col flex-1 font-secondary overflow-y-auto min-h-screen"
            ...
            <div class="px-4 md:px-6 lg:px-8">
                ...
                <template x-if="showShiftWarning">
                    <div
                        class="fixed top-20 right-5 left-5 md:left-auto md:w-[360px] bg-yellow-50 border border-yellow-200 text-yellow-900 px-4 py-3 rounded-md shadow z-50">
                        <div class="flex items-start gap-2">
                            <div class="mt-0.5"><i class="fas fa-exclamation-triangle"></i></div>
                            <div class="flex-1">
                                <div class="font-semibold">Shift akan berakhir</div>
                                <div class="text-sm">Anda akan logout otomatis dalam <span class="font-mono font-bold"
                                        x-text="formatSeconds(shiftCountdownSec)"></span>. Selesaikan transaksi Anda.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            ...
                            <div class="mt-1 relative">
                                ...
                                <input x-ref="barcodeInput" x-model="barcode" @input="onInput()"
                                    @keydown.enter.prevent="highlightedIndex >= 0 ? selectHighlighted() : scanBarcode()"
                                    @keydown.arrow-down.prevent="moveHighlight(1)"
                                    @keydown.arrow-up.prevent="moveHighlight(-1)"
                                    @keydown.escape.prevent="closeSuggestions()" :disabled="isLoading"
                                ...
                                <div x-show="showSuggestions && !isLoading" x-transition
                                    @click.outside="closeSuggestions()"
                                    class="absolute left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg z-20 max-h-72 overflow-auto">
                                    <template x-for="(p,idx) in suggestions" :key="p.id">
                                        <div :id="'sug-' + idx" @click="selectSuggestion(p)"
                                            @mouseenter="highlightedIndex = idx" class="px-3 py-2 cursor-pointer"
                                            :class="highlightedIndex === idx ? 'bg-primary/10' : 'hover:bg-gray-50'">
                                            <div class="flex items-center gap-3">
                                                <img :src="getProductImage(p)"
                                                    @error="$event.target.src = getProductImage({})" alt="thumb"
                                                    class="w-8 h-8 rounded object-cover border" />
                                                <div class="flex-1 min-w-0">
                                                    <div class="text-sm font-medium truncate" x-text="p.product_name">
                                                    </div>
                                                    <div class="text-[11px] text-gray-500 truncate" x-text="p.barcode">
                                                    </div>
                                                </div>
                                                <div class="text-xs font-semibold text-primary"
                                                    x-text="formatCurrency(p.price || 0)"></div>
                                            </div>
                                        </div>
                                    </template>
                            ...
                            <div
                                class="mt-4 overflow-x-auto max-h-[55vh] overflow-y-auto rounded-lg border border-gray-100">
                                <table class="w-full min-w-max">
                                    <thead class="bg-primary text-white sticky top-0 z-10">
                                        <tr>
                                            <th class="px-3 py-2 text-left text-sm font-semibold first:rounded-tl-lg">
                                                Produk</th>
                                            ...
                                            <th class="px-3 py-2 text-center text-sm font-semibold last:rounded-tr-lg">
                                                Aksi</th>
                                        </tr>
                                    </thead>
                                                ...
                                                <td colspan="5" class="text-center py-8 text-gray-500">
                                                    <div
                                                        ...
                                                        <img src="<?= base_url('images/illustration/nodata.png') ?>"
                                                            class="w-24 md:w-36 h-auto mb-2 object-contain"
                                                            alt="No Data">
                                        ...
                                        <template x-for="(it, idx) in paginatedCart" :key="it.product_id">
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-3 py-2 text-right font-medium"
                                                    x-text="formatCurrency(it.price)"></td>
                                                <td class="px-3 py-2 text-center">
                                                    <div
                                                        class="inline-flex items-center bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden divide-x divide-gray-200">
                                                        <button @click="adjustQty(it.product_id, -1)"
                                                            :disabled="isLoading" aria-label="Kurangi"
                                                        ...
                                                        <button @click="adjustQty(it.product_id, 1)"
                                                            :disabled="isLoading" aria-label="Tambah"
                                                            class="h-8 w-8 grid place-content-center text-primary hover:bg-primary/5 disabled:opacity-50">
                                                ...
                                                <td class="px-3 py-2 text-right font-semibold"
                                                    x-text="formatCurrency(it.price * it.quantity)"></td>
                                                <td class="px-3 py-2 text-center">
                                                    <button @click="removeItem(idx + ((cartPage-1)*cartPageSize))"
                                                        :disabled="isLoading"
                            ...
                            <div
                                ...
                                <div class="text-xs sm:text-sm text-gray-600">
                                    <span class="hidden sm:inline">
                                        ...
                                        <span class="font-semibold"
                                            x-text="filteredCart.length === 0 ? 0 : ((cartPage - 1) * cartPageSize) + 1"></span>
                                        hingga
                                        <span class="font-semibold"
                                            x-text="Math.min(cartPage * cartPageSize, filteredCart.length)"></span>
                                        dari
                                ...
                                <div class="flex items-center gap-2" x-show="totalCartPages > 1">
                                    <button @click="changeCartPage(cartPage - 1)" :disabled="cartPage === 1"
                                    ...
                                    <template x-for="page in getCartPageNumbers()" :key="page">
                                        <button @click="changeCartPage(page)"
                                    ...
                                    <button @click="changeCartPage(cartPage + 1)"
                        ...
                        <div class="w-full md:w-80">
                            <div class="p-4 border-t md:border-t-0 md:border-l border-gray-100">
                                ...
                                <div class="mb-3" x-show="paymentMethod === 'cash'">
                                    ...
                                    <input type="number" x-model.number="payment" :disabled="isLoading"
                                        class="w-full border rounded px-3 py-2 mt-1 disabled:opacity-50">
                                </div>

                                <div class="mb-3" x-show="paymentMethod === 'cash'">
                                    <div class="text-sm text-gray-600">Kembalian</div>
                                    <div class="text-lg font-medium" x-text="formatCurrency(change)"
                                ...
                                <button @click="submitTransaction()"
                                    ...
                                    <span
                                        x-text="isLoading ? 'Memproses...' : (paymentMethod === 'qris' ? 'Generate QRIS' : 'Bayar')"></span>
                                </button>
