...
<main>
    ...
    <div class="flex">
        ...
        <div class="flex flex-col flex-1 font-secondary overflow-y-auto min-h-screen" x-data="shiftManagement()"
            x-init="fetchData()">
            ...
            <div class="mt-4 md:mt-6 lg:mt-8 px-4 md:px-6 lg:px-8 pb-20 flex flex-col 2xl:flex-row gap-5">
                <div class="flex flex-col space-y-2 w-full lg:flex-1">
                    <div class="flex justify-between items-center">
                        <h1 class="font-bold text-lg text-gray-700">Daftar Kasir</h1>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <div
                            ...
                            <div class="text-xs sm:text-sm text-gray-600">
                                <span class="hidden sm:inline">
                                    ...
                                    <span class="font-semibold"
                                        x-text="cashiers.length === 0 ? 0 : ((dataCashierPage - 1) * dataCashierPageSize) + 1"></span>
                                    hingga
                                    <span class="font-semibold"
                                        x-text="Math.min(dataCashierPage * dataCashierPageSize, cashiers.length)"></span>
                                    dari
                            ...
                            <div class="flex items-center gap-2" x-show="totalCashierPages > 1">
                                <button @click="changeDataCashierPage(dataCashierPage - 1)"
                                ...
                                <template x-for="page in getDataCashiersNumber()" :key="page">
                                    <button @click="changeDataCashierPage(page)"
                    ...
                    <div class="flex flex-col space-y-2">
                        ...
                        <div class="flex justify-between items-center">
                            <h1 class="font-bold text-lg text-gray-700"
                                x-text="showTrashShifts ? 'Sampah Shift' : 'Daftar Shift'"></h1>
                        ...
                        <div x-show="showTrashShifts" x-cloak x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 transform translate-x-4"
                            x-transition:enter-end="opacity-100 transform translate-x-0"
                            class="bg-white rounded-lg shadow-md overflow-hidden">
                            <div class="p-4 border-b border-gray-200 flex justify-between items-center bg-red-50">
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" @change="toggleSelectAllTrashShifts($event)"
                                        class="rounded border-gray-300 text-primary focus:ring-primary">
                                    <span class="text-sm text-gray-600"
                                        x-text="selectedTrashShifts.length + ' dipilih'"></span>
                                </div>
                                <div class="flex gap-2" x-show="selectedTrashShifts.length > 0">
                                    <button @click="restoreSelectedShifts()"
                                    ...
                                    <button @click="deletePermanentSelectedShifts()"
                                        ...
                                        <template x-for="shift in paginatedTrashShifts" :key="shift.id">
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-3 text-sm font-medium text-gray-900"
                                                    x-text="shift.name"></td>
                                                <td class="px-4 py-3 text-sm text-center text-gray-500"
                                                    x-text="formatDateTime(shift.deleted_at)"></td>
                                                <td class="px-4 py-3 text-sm text-center">
                                                    <div class="flex justify-center gap-2">
                                                        <button @click="confirmRestoreShift(shift.id)"
                                                            class="text-green-600 hover:text-green-800"
                                                            title="Pulihkan">
                                                            <i class="fas fa-undo"></i>
                                                        </button>
                                                        <button @click="confirmDeletePermanentShift(shift.id)"
                            ...
                            <div
                                ...
                                <div class="text-xs text-gray-600">
                                    <span class="font-semibold"
                                        x-text="trashShifts.length === 0 ? 0 : ((trashShiftsPage - 1) * trashShiftsPageSize) + 1"></span>
                                    -
                                    <span class="font-semibold"
                                        x-text="Math.min(trashShiftsPage * trashShiftsPageSize, trashShifts.length)"></span>
                                    dari
                                    <span class="font-semibold" x-text="trashShifts.length"></span>
                                </div>

                                <div class="flex items-center gap-1" x-show="totalTrashShiftsPages > 1">
                                    <button @click="changeTrashShiftsPage(trashShiftsPage - 1)"
                                    ...
                                    <template x-for="page in getTrashShiftsPageNumbers()" :key="page">
                                        <button @click="changeTrashShiftsPage(page)"
                        ...
                        <div x-show="!showTrashShifts" x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 transform -translate-x-4"
                            x-transition:enter-end="opacity-100 transform translate-x-0"
                            class="bg-white rounded-lg shadow-md overflow-hidden">
                            <div class="overflow-x-auto max-h-[40vh] overflow-y-auto">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        ...
                                        <template x-for="(shift, index) in paginatedShifts" :key="shift.id">
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-3 text-sm text-center"
                                                    x-text="`${shift.start_time} - ${shift.end_time}`"></td>
                                                <td class="px-4 py-3 text-sm text-center">
                                                ...
                                                <td class="px-4 py-3 text-sm">
                                                    <div class="w-full flex flex-row items-center justify-center gap-2">
                                                        <button type="button" @click="openEditShift(shift)"
                                                            title="Edit Shift" aria-label="Edit Shift"
                                                        ...
                                                        <button type="button" @click="openDeleteShift(shift)"
                                                            title="Hapus Shift" aria-label="Hapus Shift"
                            ...
                            <div
                                ...
                                <div class="text-xs text-gray-600">
                                    <span class="font-semibold"
                                        x-text="shifts.length === 0 ? 0 : ((dataShiftsPage - 1) * dataShiftsPageSize) + 1"></span>
                                    -
                                    <span class="font-semibold"
                                        x-text="Math.min(dataShiftsPage * dataShiftsPageSize, shifts.length)"></span>
                                    dari
                                    <span class="font-semibold" x-text="shifts.length"></span>
                                </div>

                                <div class="flex items-center gap-1" x-show="totalShiftsPages > 1">
                                    <button @click="changeDataShiftsPage(dataShiftsPage - 1)"
                                    ...
                                    <template x-for="page in getDataShiftsNumber()" :key="page">
                                        <button @click="changeDataShiftsPage(page)"
