...
<div x-cloak x-show="openDeleteProductModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-center gap-4">
            <button @click="openDeleteProductModal = false"
                class="px-4 py-2 rounded-md border border-gray-200 text-gray-700 hover:bg-gray-100">
                Batal
            </button>
            <button @click="deleteProduct(selectedProduct.id)" :disabled="isDeletingProduct"
                class="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2">
...
<div x-cloak x-show="openDeleteCategoryModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    <div class="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-center gap-4">
            <button @click="openDeleteCategoryModal = false"
                class="px-4 py-2 rounded-md border border-gray-200 text-gray-700 hover:bg-gray-100">
                Batal
            </button>
            <button @click="deleteCategory(selectedCategory.id)" :disabled="isDeletingCategory"
                class="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2">
...
<!-- Add Product Modal -->
<div x-cloak x-show="openAddProductModal" @click.self="openAddProductModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openAddProductModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    <form @submit.prevent="addProduct" class="w-full max-w-xl bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-5 grid grid-cols-1 gap-4">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" placeholder="Masukkan nama produk"
                        ...
                        x-model="selectedProduct.product_name" required>
                    <p class="text-red-500 text-xs italic mt-1" x-show="validationErrors.product_name"
                        x-text="validationErrors.product_name"></p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" placeholder="Masukkan harga"
                        class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition"
                        x-model="selectedProduct.price" min="0" required>
                    <p class="text-red-500 text-xs italic mt-1" x-show="validationErrors.price"
                        x-text="validationErrors.price"></p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select
                        ...
                        x-model="selectedProduct.category_id" required>
                        <option value="" disabled>Pilih kategori</option>
                        <template x-for="category in categories" :key="category.id">
                            <option :value="category.id" x-text="category.category_name"></option>
                        </template>
                    </select>
                    <p class="text-red-500 text-xs italic mt-1" x-show="validationErrors.category_id"
                        x-text="validationErrors.category_id"></p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="file" accept="image/*" @change="handlePhotoUpload($event, true)"
                ...
                <template x-if="selectedProduct.photo">
                    <div class="mt-2">
                        <img :src="getPhotoPreview(selectedProduct.photo)" alt="Preview"
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-end gap-3">
            <button type="button" @click="openAddProductModal = false"
...
<!-- Edit Product Modal -->
<div x-cloak x-show="openEditProductModal" @click.self="openEditProductModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openEditProductModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    <form @submit.prevent="editProduct" class="w-full max-w-xl bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-5 grid grid-cols-1 gap-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <template x-if="selectedProduct?.photo">
                        <div>
                            <img :src="selectedProduct.photo" alt="Current Photo"
                                @error="(e) => e.target.src = '<?= base_url('images/illustration/deletemodal-illustration.png') ?>'"
                                class="w-20 h-20 object-cover rounded">
                        </div>
                    </template>
                    <div class="flex-1">
                        <input type="file" accept="image/*" @change="handlePhotoUpload($event, false)"
                            class="w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition">
                    </div>
                </div>
            </div>
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-end gap-3">
            <button type="button" @click="openEditProductModal = false"
                class="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-100 transition">
                Batal
            </button>
            <button type="submit"
...
<!-- Add Category Modal -->
<div x-cloak x-show="openAddCategoryModal" @click.self="openAddCategoryModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openAddCategoryModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" placeholder="Masukkan nama kategori"
                        class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition"
                        x-model="selectedCategory.category_name" required>
                    <p class="text-red-500 text-xs italic mt-1" x-show="validationErrors.category_name"
                        x-text="validationErrors.category_name"></p>
                </div>
            </div>
        </div>
...
<!-- Edit Category Modal -->
<div x-cloak x-show="openEditCategoryModal" @click.self="openEditCategoryModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openEditCategoryModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
...
<div x-cloak x-show="openViewProductModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
        ...
        <div class="px-5 py-4">
            <div class="flex items-center gap-4">
                <div class="w-20 h-20 bg-gray-100 rounded flex items-center justify-center overflow-hidden">
                    ...
                    <template x-if="!selectedProduct?.photo">
                        <img src="<?= base_url('images/placeholder-product.svg') ?>" alt="No photo"
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Kategori: <span
                            x-text="selectedProduct?.category_name || '-'"></span></p>
            ...
            <div class="mt-4">
                        ...
                        <table class="w-full min-w-max text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <template x-if="!(selectedProduct && selectedProduct.batches && selectedProduct.batches.length) ||
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-3 py-4 text-center text-gray-500">Tidak ada batch
                                            aktif yang ditampilkan</td>
                                    </tr>
                                </template>
                                ...
                                <template x-for="b in (selectedProduct?.batches || []).filter(b => {
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-2 text-right" x-text="formatCurrency(b.purchase_price || 0)">
                                        </td>
                                        <td class="px-3 py-2">
                ...
                <div class="text-xs text-gray-500 mt-1">Catatan: batch yang expired atau near-expired (≤ 7 hari) tidak
                    ditampilkan.</div>
            </div>
            ...
            <div class="mt-4 flex flex-col items-center">
                <div class="bg-white p-4 rounded-md shadow-inner min-h-[100px] flex items-center justify-center">
                    <template x-if="selectedProduct?.barcode">
                        <img :src="barcodeImageUrl || '/admin/products/barcode/image/' + encodeURIComponent(selectedProduct.barcode)"
                            alt="Barcode" class="w-56 h-auto">
                    </template>
                    <template x-if="!selectedProduct?.barcode">
                        <span class="text-gray-400 italic">Barcode tidak tersedia</span>
                    </template>
                </div>
                <p class="text-sm text-gray-600 mt-2">Kode: <span class="font-mono"
                        x-text="selectedProduct?.barcode || '-'"></span></p>
            </div>
        </div>

        <div class="px-5 py-4 bg-gray-50 flex flex-col sm:flex-row items-center justify-center gap-3">
            <template x-if="selectedProduct?.barcode">
                <a :href="barcodeImageUrl || '/admin/products/barcode/image/' + encodeURIComponent(selectedProduct.barcode)"
                    target="_blank"
                    class="w-full sm:w-auto px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-100 text-center">Buka
                    Gambar</a>
            </template>
            <button @click="downloadBarcode()" :disabled="!selectedProduct?.barcode"
                class="w-full sm:w-auto px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed">Unduh
                Barcode</button>
            <button @click="openViewProductModal = false"
                class="w-full sm:w-auto px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-100">Tutup</button>
        </div>
    </div>
</div>

<!-- Modal Konfirmasi Persetujuan Restock -->
<div x-cloak x-show="approveModalOpen" @click.self="closeApproveRestock()"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
...
<div x-cloak x-show="approveModalOpen" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="w-full max-w-xl bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div class="px-5 py-5 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
            ...
            <div class="space-y-1 col-span-2">
                ...
                <p class="font-medium text-gray-800 break-words" x-text="restockDetails.note || '-'"></p>
            </div>
...
<div x-cloak x-show="openDeleteLocationModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
        ...
        <div class="px-5 py-6 flex flex-col items-center text-center gap-3">
            ...
            <p class="text-gray-700">
                ...
                <span class="font-semibold"
                    x-text="(selectedLocation.rack || '') + ' - ' + (selectedLocation.row || '')"></span>?
            </p>
            <p class="text-sm text-gray-500">Tindakan ini tidak dapat dibatalkan.</p>
        </div>
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-center gap-4">
            <button @click="openDeleteLocationModal = false"
                class="px-4 py-2 rounded-md border border-gray-200 text-gray-700 hover:bg-gray-100">
                Batal
            </button>
            <button @click="deleteLocation(selectedLocation.id)" :disabled="isDeletingLocation"
                class="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2">
...
<!-- Add Location Modal -->
<div x-cloak x-show="openAddLocationModal" @click.self="openAddLocationModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openAddLocationModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    <form @submit.prevent="addLocation" class="w-full max-w-xl bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <div class="relative">
                    <input type="text" placeholder="Contoh: A, B, R1"
                        ...
                        x-model="selectedLocation.rack" required>
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Baris/Level</label>
                <div class="relative">
                    <input type="text" placeholder="Contoh: 1, 2, L1"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition"
                        x-model="selectedLocation.row" required>
                </div>
            </div>
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-end gap-3">
            <button type="button" @click="openAddLocationModal = false"
                class="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-100 transition">
                Batal
            </button>
            <button type="submit" :disabled="isSavingLocation || !selectedLocation.rack || !selectedLocation.row"
                class="px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90 focus:ring-2 focus:ring-primary/30 transition disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2">
...
<!-- Edit Location Modal -->
<div x-cloak x-show="openEditLocationModal" @click.self="openEditLocationModal = false"
    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    class="fixed inset-0 bg-black/40 backdrop-blur-xs z-50" aria-hidden="true"></div>
<div x-cloak x-show="openEditLocationModal" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    <form @submit.prevent="editLocation" class="w-full max-w-xl bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="px-5 py-4 bg-gray-50 flex items-center justify-end gap-3">
            <button type="button" @click="openEditLocationModal = false"
                class="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-100 transition">
                Batal
            </button>
            <button type="submit" :disabled="isSavingLocation || !selectedLocation.rack || !selectedLocation.row"
...
<div x-cloak x-show="openProofModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    <div class="w-full max-w-lg bg-white rounded-xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="p-5 overflow-y-auto">
            <template x-if="selectedRestock?.receipt_image">
                <div class="mb-4">
                    ...
                    <img :src="selectedRestock.receipt_image" class="w-full rounded border border-gray-200"
                        alt="Bukti Restock">
                </div>
            </template>
            <template x-if="selectedRestock?.user_note">
                <div>
                    ...
                    <p class="p-3 bg-gray-50 rounded border border-gray-200 text-gray-700 whitespace-pre-wrap"
                        x-text="selectedRestock.user_note"></p>
                </div>
            </template>
        ...
        <div class="px-5 py-4 bg-gray-50 flex justify-end shrink-0">
            <button @click="openProofModal = false"
                class="px-4 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300">Tutup</button>
        </div>
    </div>
</div>